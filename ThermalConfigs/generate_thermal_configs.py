...
THERMAL_CONFIG_REQUEST_PROP = "persist.sys.pixelparts.thermal_config_request"
THERMAL_CONFIG_DATA_DIR = "/data/pixelparts/ThermalConfigs"
THERMAL_CONFIG_VENDOR_DIR = "/data/vendor/pixelparts/ThermalConfigs"
THERMAL_CONFIG_ACTIVE_FILE = "active.json"
THERMAL_CONFIG_ACTIVE_PATH = f"{THERMAL_CONFIG_VENDOR_DIR}/{THERMAL_CONFIG_ACTIVE_FILE}"
...
def write_text_if_changed(path, content):
    ...
    if path.exists():
        existing = read_text(path).replace("\r\n", "\n").replace("\r", "\n")
...
def write_copy_rules(generated_base_names, destination_dir):
    ...
    lines = [
        ...
        f"# Generated by script in: {REL_SCRIPT_PATH}",
        "",
    ]

    if not generated_base_names:
        lines.append("# Thermal config generation is disabled for this target.")
        write_text_if_changed(copy_rules_path, "\n".join(lines) + "\n")
        return
    ...
    lines.append(f"    {last_source}:{last_destination}")
    write_text_if_changed(copy_rules_path, "\n".join(lines) + "\n")
        ...
        if normalized in (
            ...
            while index < len(lines):
                ...
                if (
                    ...
                    or next_normalized.startswith(f"copy {THERMAL_CONFIG_DATA_DIR}/${{{THERMAL_CONFIG_PROP}}}")
                    or next_normalized.startswith(f"chown system system {THERMAL_CONFIG_ACTIVE_PATH}")
                    or next_normalized.startswith(f"chmod 0644 {THERMAL_CONFIG_ACTIVE_PATH}")
                    or next_normalized.startswith(f"restorecon {THERMAL_CONFIG_ACTIVE_PATH}")
...
def update_init_rc(init_rc_path, enabled):
    ...
    if enabled:
        switch_lines = [
            f"on property:{THERMAL_CONFIG_REQUEST_PROP}=* && property:{THERMAL_CONFIG_PROP}=*",
            f"    copy {THERMAL_CONFIG_DATA_DIR}/${{{THERMAL_CONFIG_PROP}}} {THERMAL_CONFIG_ACTIVE_PATH}",
            f"    chown system system {THERMAL_CONFIG_ACTIVE_PATH}",
            f"    chmod 0644 {THERMAL_CONFIG_ACTIVE_PATH}",
            f"    restorecon {THERMAL_CONFIG_ACTIVE_PATH}",
            f"    setprop vendor.thermal.config ${{{THERMAL_CONFIG_REQUEST_PROP}}},{THERMAL_CONFIG_ACTIVE_PATH}",
            "",
            f"on property:{THERMAL_CONFIG_REQUEST_PROP}=* && property:{THERMAL_CONFIG_PROP}={BASE_CONFIG_FILENAME}",
            f"    setprop vendor.thermal.config {BASE_CONFIG_FILENAME}",
        ]
    else:
        switch_lines = [
    ...
    write_text_if_changed(init_rc_path, updated_content)
...
def generate_configs(source_path, device_codename, destination_dir):
    ...
    for soc_name, soc_offset in OFFSETS.items():
        for battery_name, battery_offset in OFFSETS.items():
            ...
            write_text_if_changed(target_path, new_content)
