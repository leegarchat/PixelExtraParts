...
public class AddonLoader {
    ...
    // Key for manual restart flag — set by SystemUIRestartUtils before user-initiated restart
    private static final String MANUAL_RESTART_FLAG = "pixel_addon_manual_restart";
    ...
    public static void loadAndRunAddons(Context context, ClassLoader appClassLoader, String packageName) {
        ...
        // Manual restart flag: skip boot guard for user-initiated restarts
        boolean guarded = isGuardedPackage(packageName);
        if (guarded && consumeManualRestartFlag(context)) {
            Log.d(TAG, "Manual restart detected — clearing boot guard for " + packageName);
            clearBootGuard(context);
            // Don't increment counter, proceed with addons
        } else if (guarded && checkAndUpdateBootGuard(context)) {
    ...
    /**
     * Check and consume the manual restart flag set by SystemUIRestartUtils.
     * Returns true if the flag was set (indicating a user-initiated restart).
     * The flag is cleared after reading so it only applies once.
     */
    private static boolean consumeManualRestartFlag(Context context) {
        try {
            int flag = Settings.Global.getInt(
                context.getContentResolver(), MANUAL_RESTART_FLAG, 0);
            if (flag == 1) {
            ...
            Log.d(TAG, "Failed to read manual restart flag: " + throwableSummary(t));
        }
        return false;
    }
