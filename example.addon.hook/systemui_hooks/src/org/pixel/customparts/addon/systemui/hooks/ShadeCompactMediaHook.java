...
public class ShadeCompactMediaHook extends BaseSystemUIHook {
	...
	private float getPlayerAlpha(Context context) {
		...
		float a = 1f;
		try {
			// Preferred: addon base helper (Settings.Global + _pine suffix)
	...
	private Mode getMode(Context context) {
		...
		// 1) Preferred: addon base helper (Settings.Global + _pine suffix) as an int mode.
		try {
		...
		if (modeValue == 0) {
			try {
				modeValue = Settings.System.getInt(
		...
		if (modeValue == 0) {
			try {
				int nativeEnabled = Settings.System.getInt(
					context.getContentResolver(),
