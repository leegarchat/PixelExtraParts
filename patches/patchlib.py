...
class SnapshotPatch(BasePatch):
    ...
    def __init__(
        ...
        self.source_path = f"packages/apps/PixelExtraParts/patches/files/modified/{target}"
...
class NewFilePatch(BasePatch):
    ...
    def __init__(
        ...
        self.source_path = f"packages/apps/PixelExtraParts/patches/files/new/{target}"
        self.bypass_by_default = bypass_by_default
...
class ActivityThreadInjectPatch(BasePatch):
    ...
    def run(self, context: PatchContext) -> PatchResult:
        target_path = context.resolve(self.target)
        # source now lives in files/modified
        mod_base = Path(__file__).resolve().parent / "files" / "modified"
        source_path = mod_base / self.target
