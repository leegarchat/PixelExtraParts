...
constexpr std::string_view kDefaultConfigPathPrefix("/vendor/etc/");
...
std::string ResolveThermalConfigPath(std::string_view config_path) {
    if (::android::base::StartsWith(config_path, "/")) {
        return std::string(config_path);
    }

    return std::string(kDefaultConfigPathPrefix) + std::string(config_path);
}
...
bool ParseThermalConfig(std::string_view config_path, Json::Value *config,
    ...
    for (Json::Value::ArrayIndex i = 0; i < sub_configs_paths.size(); ++i) {
        const std::string sub_configs_path =
            ResolveThermalConfigPath(sub_configs_paths[i].asString());
