...
void ThermalWatcher::registerFilesToWatch(const std::set<std::string> &sensors_to_watch) {
    ...
    updateMonitoredSensors(sensors_to_watch);
...
void ThermalWatcher::updateMonitoredSensors(const std::set<std::string> &sensors_to_watch) {
    std::lock_guard<std::mutex> lock(monitored_sensors_mutex_);
    monitored_sensors_ = sensors_to_watch;
}

bool ThermalWatcher::isMonitoredSensor(std::string_view name) {
    std::lock_guard<std::mutex> lock(monitored_sensors_mutex_);
    return monitored_sensors_.find(name.data()) != monitored_sensors_.end();
}
...
void ThermalWatcher::parseGenlink(std::unordered_map<std::string, float> *sensor_map) {
    ...
    while (!done && !err) {
        ...
        std::string name;
        if (getThermalZoneTypeById(tz_info.first, &name) && isMonitoredSensor(name)) {
