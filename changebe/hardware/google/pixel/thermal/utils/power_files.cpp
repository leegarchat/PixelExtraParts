...
void PowerFiles::clear() {
    energy_info_map_.clear();
    power_rail_info_map_.clear();
    energy_path_set_.clear();
    prev_energy_info_map_.clear();

    std::unique_lock<std::shared_mutex> _lock(power_status_map_mutex_);
    power_status_map_.clear();
}
