...
struct PowerHintstatus {
  ...
  ThrottlingSeverity prev_hint_severity = ThrottlingSeverity::NONE;
};
...
class PowerHalService {
  ...
  static void onPowerHalExtAidlBinderDied(void *cookie) {
    ...
    auto *service = static_cast<PowerHalService *>(cookie);
    {
      std::lock_guard<std::mutex> lock(service->lock_);
      service->power_hal_aidl_ = nullptr;
      service->power_hal_ext_aidl_ = nullptr;
    }
    service->reconnect();
  }
